...
const AccordionItem = ({ title, icon, text }) => {
  ...
  return (
    <div className="min-w-[500px] bg-primary-50 p-6 text-center rounded-[15px] flex  flex-col items-center box">
        <button
        onClick={toggle}
        ...
        {isOpen && <div className="max-w-[400px] text-start leading-5 py-8 text-primary-800">{text}</div>}
    </div>
  )
}
...
const Accordion = ({ items }) => {
    return (
        <div className="flex w-full flex-wrap justify-start items-start gap-8">
            {items.map((item) => (
                <AccordionItem key={item.title} title={item.title} text={item.text} icon={item.icon} />
            ))}
        </div>
    )
}
